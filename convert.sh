...
# TODO: Convert CALS tables to html?
...
do
    ...
    # Cleanup temp file:
    rm $OUTPUT.tmp 
    echo "<!ENTITY ${BASE} SYSTEM \"${BASE}.xml\">" >> $BOOK_FILE
done

cat >> $BOOK_FILE <<EOF
]>
<book version="5.0" xml:id="os-security-guide" xmlns="http://docbook.org/ns/docbook" xmlns:xlink="http://www.w3.org/1999/xlink">
...
for file in `find $SOURCE_DIR -iname '*.html' -print`;
do
    BASE=`basename $file .html`
    echo "&${BASE};" >> $BOOK_FILE
done
